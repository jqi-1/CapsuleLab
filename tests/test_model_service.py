...
def test_register_list_verify_and_remove_model(tmp_path, monkeypatch):
    monkeypatch.setattr(model_service, "MODEL_STORAGE", tmp_path / "models.json")
...
def test_model_api_maps_missing_artifact_to_404(tmp_path, monkeypatch):
    ...
    try:
        models.register_model(models.RegisterModelRequest(name="missing", version="1", path=str(tmp_path / "missing.bin")))
