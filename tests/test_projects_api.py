...
from backend.api import environment, ide, projects
...
def test_import_project_endpoint_uses_git_import(monkeypatch):
    ...
    monkeypatch.setattr(projects.project_import_service, "import_project", fake_import)
...
def test_delete_project_removes_existing_inventory_row(monkeypatch):
    ...
    monkeypatch.setattr(
        "capsulelab.db.repositories.projects.get",
        lambda project_id: {"id": project_id, "name": "demo", "path": "/tmp/demo"},
    )
...
def test_setup_ide_endpoint_uses_project_path(monkeypatch):
    ...
    monkeypatch.setattr(ide.ide_service, "setup_ide", fake_setup)

    result = ide.setup_ide("cap-demo", ide.IdeSetupRequest(ide="cursor"))
...
def test_ide_instructions_endpoint_maps_unknown_ide(monkeypatch):
    ...
    monkeypatch.setattr(ide.ide_service, "attach_instructions", fake_instructions)

    try:
        ide.ide_instructions("cap-demo", "unknown")
    except HTTPException as exc:
...
def test_project_status_returns_full_status(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "load_config",
        lambda path: ProjectConfig(name="demo", runtime=RuntimeConfig(image="demo:dev")),
    )
    monkeypatch.setattr(projects.project_service, "validate", lambda config, path: [])
    monkeypatch.setattr(
        projects.docker_service,
        "check_docker_status",
        lambda: type(
            "DockerStatus",
            (),
    ...
    monkeypatch.setattr(
        projects.gpu_service,
        "get_gpu_info",
        lambda: type("GpuInfo", (), {"available": False, "name": "", "vram_mb": 0})(),
    )
    monkeypatch.setattr(
        projects.git_service,
        "git_status",
        lambda path: {"is_repo": False, "branch": "", "remote": "", "dirty_files": 0, "lfs_available": False},
    )
    monkeypatch.setattr(
        projects.resource_service,
        "project_resources",
        lambda path: {
...
def test_project_environment_endpoint_uses_project_path(monkeypatch):
    ...
    monkeypatch.setattr(
        environment.environment_service, "describe", lambda path: {"path": path, "dependencies": [], "environment": {}}
    )

    result = environment.project_environment("cap-demo")
...
def test_add_project_dependency_endpoint_maps_validation_error(monkeypatch):
    ...
    monkeypatch.setattr(environment.environment_service, "add_dependency", fake_add)

    try:
        environment.add_project_dependency("cap-demo", environment.AddDependencyRequest(dependency=""))
    except HTTPException as exc:
        assert exc.status_code == 400
    else:
        raise AssertionError("Expected HTTPException")
...
def test_set_project_environment_variable_endpoint(monkeypatch):
    ...
    monkeypatch.setattr(environment.environment_service, "set_environment_variable", fake_set)

    result = environment.set_project_environment_variable(
        "cap-demo",
        environment.EnvironmentVariableRequest(name="API_URL", value="https://example.test"),
    )
