...
def get_resource_history(project_id: str, limit: int = Query(100, ge=1, le=1000)):
    """Get historical resource usage for a project"""
    _project(project_id)
    history = resource_service.get_resource_history(project_id, limit)
    return {"project_id": project_id, "history": history, "count": len(history)}
...
def collect_resource_snapshot(project_id: str):
    ...
    # Combine resources
    combined_resources = {**system_resources, **project_resources}

    # Store snapshot
    snapshot_id = resource_service.store_resource_snapshot(project_id, combined_resources)

    return {"project_id": project_id, "snapshot_id": snapshot_id, "message": "Resource snapshot collected successfully"}
