...
from backend.db.sqlite import list_locations, get_location_by_name, register_location, remove_location, set_location_override, remove_location_override, list_location_overrides
...
def create_location(req: CreateLocationRequest):
    existing = get_location_by_name(req.name)
    if existing:
        raise HTTPException(409, f"Location '{req.name}' already exists")
    from uuid import uuid4
    location_id = str(uuid4())
    register_location(location_id, req.name, "ssh", req.host, req.user, req.project_root, req.runtime, req.gpu)
    loc = get_location_by_name(req.name)
    if loc:
...
def add_location_override(name: str, req: SetOverrideRequest):
    ...
    set_location_override(loc["id"], req.override_type, req.logical_name, req.value)
...
def get_location_overrides(name: str):
    ...
    if not loc:
        raise HTTPException(404, "Location not found")
    return list_location_overrides(loc["id"])
