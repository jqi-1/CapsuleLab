...
def validate_template(name: str, base_dir: Path | None = None) -> list[TemplateCheck]:
    ...
    if dockerfile.exists():
        ...
        checks.append(
            TemplateCheck(
                ...
                dockerfile_text.startswith("FROM "),
                "Starts with FROM" if dockerfile_text.startswith("FROM ") else "Missing FROM",
            )
        )
        checks.append(
            TemplateCheck(
                "Dockerfile workdir",
                "WORKDIR" in dockerfile_text,
                "Found" if "WORKDIR" in dockerfile_text else "Missing WORKDIR",
            )
        )
        checks.append(
            TemplateCheck(
                "Dockerfile command", "CMD" in dockerfile_text, "Found" if "CMD" in dockerfile_text else "Missing CMD"
            )
        )
    ...
    checks.append(
        TemplateCheck(
            "Project config",
            config_path.exists(),
            "Found" if config_path.exists() else "Missing .workbench/project.yaml",
        )
    )
    ...
    checks.append(
        TemplateCheck(
            "README",
            readme.exists() and bool(readme.read_text().strip()) if readme.exists() else False,
            "Found" if readme.exists() else "Missing",
        )
    )
    ...
    checks.append(
        TemplateCheck(
            "Requirements",
            requirements.exists() and bool(requirements.read_text().strip()) if requirements.exists() else False,
            "Found" if requirements.exists() else "Missing",
        )
    )

    starter_files = (
        list((template_path / "notebooks").glob("*.ipynb")) if (template_path / "notebooks").exists() else []
    )
    ...
    checks.append(
        TemplateCheck(
            ...
            bool(starter_files),
            ", ".join(str(p.relative_to(template_path)) for p in starter_files)
            if starter_files
            else "Missing starter notebook or app",
        )
    )
    ...
    if config:
        if config.apps:
            for app in config.apps:
                ...
                detail = (
                    f"{app.id}: {app.command} on {app.port or 'no port'}"
                    if ok
                    else f"Invalid app entry: {app.model_dump()}"
                )
